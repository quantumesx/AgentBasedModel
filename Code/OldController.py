"""Contains defunct code for simple feedforward controller."""
...
class MN_controller():
    ...
    def propagate(self, inputs, comm_unit_weight=False, validate=False):
        ...
        def get_sensory_activation(n, signal):
            ...
            if n == 13:
                print('comm_self signal:{}, activation:{}'.format(signal,
                                                                  activation))
        ...
        if comm_unit_weight:
            ...
            input = self.nodes[comm_unit]['activation'][-2]
            weight = self.connections[comm_unit]['weight']
            comm_self_signal += input * weight
        else:
            # method 2: treat comm_unit as sensor (no weight)
            input = self.nodes[comm_unit]['activation'][-2]
            print('uh comm_self input is', input)
